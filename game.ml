...
let dice_results num_offense_dies num_defense_dies =
  ...
  in
  let offense_dies = List.rev 
      (List.sort compare (get_rolls num_offense_dies [])) 
  in
  let defense_dies = List.rev 
      (List.sort compare (get_rolls num_defense_dies [])) 
  in
  let rec cmp off_lst def_lst off_lost def_lost =
    let len_tuple = (List.length off_lst, List.length def_lst) 
    in
    if fst len_tuple = snd len_tuple
    then 
      match (off_lst, def_lst) with
      | ([],_) -> (off_lost, def_lost)
      | (_,[]) -> (off_lost, def_lost)
      | (o :: t1, d :: t2) -> 
        begin
      ...
      | (2,3) -> 
        begin
          ...
          in
          if List.length count_2 > 0 
          then (fst ans, 1 + snd ans) 
...
let attack state from towards =
  ...
  let rec attack_until off def curr_state =
    ...
    | (o,d) -> 
      begin
        let get_dice_res = dice_results (fst dice_counts) (snd dice_counts) 
        in
