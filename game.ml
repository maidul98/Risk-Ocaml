...
(** [region_bonus l n] calculates the regional bonuses given a list of regions
    [lst] and the number [num], initially zero *)
and region_bonus lst num =
  match lst with
  | [] -> num
  | h :: t -> begin
      match h with
...
(** [next_player g] sets the next player in [g] as the current player *)
...
(** [two_off_dice_three_def_dice o d] is when length of [o] is two and length
    of [d] is three in [dice_results_helper] *)
let two_off_dice_three_def_dice off_lst def_lst =
...
(** [three_off_dice_two_def_dice o d] is when length of [o] is three and length
    of [d] is two in [dice_results_helper] *)
let three_off_dice_two_def_dice off_lst def_lst =
...
(** [dice_results_helper] assists [dice_results] when the values in [len_tuple]
    aren't equal *)
let dice_results_helper len_tuple off_lst def_lst =
  match len_tuple with
  | (0,_) | (_,0) -> (0,0)
  | (1,2) | (1,3) -> begin
  ...
  | (2,3) -> two_off_dice_three_def_dice off_lst def_lst
  | (3,2) -> three_off_dice_two_def_dice off_lst def_lst
  | _ -> (0,0)
...
let dice_results num_off_dies num_def_dies =
  ...
  let rec cmp off_lst def_lst off_lost def_lost =
    ...
    else dice_results_helper len_tuple off_lst def_lst
...
(** [move_troops s o f o_player d_player] moves troops from attacking territory
    to newly conquered territory *)
let move_troops state off def off_player def_player =
  if Territory.get_count off = 1 then ()
  else
    let rec move_troops num start =
...
(** [conquer_terr] assigns the newly conquered territory to the offense, removes
    ...
    invariants of the game guarantee that [off] has at least 2 troops, one to
    put in the new territory and one to keep. *)
...
let attack state from towards =
  ...
  in attack_until offense defense state |> validate_players
...
let place state count terr process_state =
  let place_msg = print_endline ("Placing " ^ string_of_int count ^
                                 " troops in " ^ terr ^ ".") in
...
(** [get_num_terr_owned s] checks how many territories in [s] are owned *)
...
(** [check_game_finish s] checks if the game in [s] is over or not *)
